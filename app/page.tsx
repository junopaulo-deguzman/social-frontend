...
export default function Home() {
  ...
  useEffect(() => {
    const feedSort = sort.toLowerCase() as "hot" | "new" | "top";
  ...
  const displayedPosts = posts
    ...
    .filter((post) =>
      `${post.title} ${post.body} ${post.community} ${post.author}`
        .toLowerCase()
    ...
    .sort((a, b) =>
      sort === "Top"
        ...
        : sort === "New"
          ? b.id - a.id
          : b.score + b.comments * 2 - (a.score + a.comments * 2),
    );
  ...
  function toggleSaved(postId: number) {
    setSaved((current) => {
      ...
      threadlyApi[next.has(postId) ? "unsave" : "save"](postId).catch(
        (error) => {
    ...
    <div className="app-shell">
      ...
      <div className="layout" id="top">
        <aside className="left-nav">
          <nav aria-label="Main navigation">
            ...
            >
              <span>◇</span>Saved <b>{saved.size || ""}</b>
            </button>
          </nav>
          <div className="nav-divider" />
          <div className="nav-heading">
            <span>Communities</span>
            <button aria-label="Add community">＋</button>
          </div>
          <div className="community-list">
            {communities.map(([letter, name, members]) => (
              <button
              ...
              >
                <span
                  className={`community-avatar community-${letter.toLowerCase()}`}
                >
                  {letter}
                </span>
                <span>
        ...
        <main className="feed">
          <section className="welcome-card">
            <div>
              ...
              <p>
                {view === "profile"
                  ? "Learning AWS in public · Building with React and Go."
                  : view === "saved"
                    ? `${saved.size} ${saved.size === 1 ? "post" : "posts"} ready when you are.`
                    : "Fresh ideas from the communities you follow."}
              </p>
            </div>
            <div className="welcome-art" aria-hidden="true">
              <span>{view === "profile" ? nameInitials : "☁"}</span>
              <b>{"{ }"}</b>
            </div>
          </section>
          <div className="feed-toolbar">
            <div className="sort-tabs" aria-label="Sort posts">
            ...
            <button className="view-button" aria-label="Change feed layout">
              ▤⌄
            </button>
          </div>
          <div className="post-list">
            {displayedPosts.map((post) => (
              <article className="post-card" key={post.id}>
                <div className="vote-column">
                  <button
                    className={votes[post.id] === 1 ? "up active" : "up"}
                  ...
                  <button
                    className={votes[post.id] === -1 ? "down active" : "down"}
                    onClick={() => vote(post.id, -1)}
                ...
                <div className="post-content">
                  <div className="post-meta">
                    ...
                    <strong>t/{post.community}</strong>
                    <span>
                      · Posted by u/{post.author} · {post.age}
                    </span>
                  </div>
                  <h2>{post.title}</h2>
                  ...
                  <div className="post-actions">
                    ...
                    <button
                      className={saved.has(post.id) ? "saved" : ""}
                      onClick={() => toggleSaved(post.id)}
                    >
                      {saved.has(post.id) ? "◆" : "◇"}{" "}
                      <span>{saved.has(post.id) ? "Saved" : "Save"}</span>
                    </button>
            ...
            {displayedPosts.length === 0 && (
              <div className="empty-state">
                ...
                <h2>
                  {view === "saved" ? "Nothing saved yet" : "No threads found"}
                </h2>
                <p>
                  {view === "saved"
                    ? "Save a useful post and it will wait for you here."
        ...
        <aside className="right-rail">
          <section className="side-card profile-card">
            ...
            <div className="profile-body">
              <div className="large-avatar">{nameInitials}</div>
              <p className="eyebrow">YOUR PROFILE</p>
              <h2>{profile?.name ?? "Loading"}</h2>
              <p className="handle">u/{profile?.username ?? "loading"}</p>
              <div className="profile-stats">
                ...
                <span>
                  <strong>{profile?.joined ?? "N/A"}</strong>
                  <small>Joined</small>
                </span>
              </div>
              <button
          ...
          <section className="side-card trending-card">
            ...
            <ol>
              ...
              <li>
                <span>02</span>
                <div>
                  <strong>Go 1.25</strong>
                  <small>8.1k posts</small>
                </div>
              </li>
              <li>
                <span>03</span>
      ...
      {showComposer && (
        <div
          ...
          role="presentation"
          onMouseDown={() => setShowComposer(false)}
        >
          <section
            className="composer"
            role="dialog"
          ...
          >
            ...
            <form onSubmit={createPost}>
              <label>
                Community
                <select name="community" defaultValue="aws-builders">
